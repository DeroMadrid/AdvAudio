...
def get_fps_tran(wav_dir, tran_txt):
    ...
    for ftr in f_tran_read:
        ...
        #                     wl_org_tran.lower()]
        name, org_tran, ds_org_tran, lv_org_tran, js_org_tran, wl_org_tran = ftr.strip('\n').split(":")  # edit by xcx
        name_tran_d[name] = [org_tran.lower(), ds_org_tran.lower(), lv_org_tran.lower(), js_org_tran.lower(), wl_org_tran.lower()]
    ...
    wl_org_tran_maxlen = max(map(len, wl_org_trans))
    wl_org_seqs = [list(np.pad([char2idx[k] for k in dot], (0, wl_org_tran_maxlen - len(dot)),
                               mode="constant", constant_values=0)) for dot in wl_org_trans]

    return fps, ds_org_seqs, ds_org_trans, lv_org_trans, js_org_seqs, js_org_trans, wl_org_seqs, wl_org_trans, \
            ds_org_tran_maxlen, js_org_tran_maxlen, wl_org_tran_maxlen, org_trans
...
def decode_extract_and_batch(wav_dir, tran_txt, shuffle=False, repeat=False, batch_size=1, decode_fs=16000,
    ...
    :return: 一个dataset
    """
    fps, ds_org_seqs, ds_org_trans, lv_org_trans, js_org_seqs, js_org_trans, wl_org_seqs, wl_org_trans, \
    ds_tran_maxlen, js_tran_maxlen, wl_tran_maxlen, org_trans = get_fps_tran(wav_dir, tran_txt)

    # fps, ds_org_seqs, ds_org_trans, lv_org_trans, js_org_seqs, js_org_trans, \
    # ds_tran_maxlen, js_tran_maxlen = get_fps_tran(wav_dir, tran_txt)
    ...
    dataset = tf.data.Dataset.from_tensor_slices((fps, ds_org_seqs, ds_org_trans, lv_org_trans, js_org_seqs, js_org_trans, wl_org_seqs, wl_org_trans, org_trans))
    # dataset = tf.data.Dataset.from_tensor_slices((fps, ds_org_seqs, ds_org_trans, lv_org_trans, js_org_seqs, js_org_trans))
    ...
    def _decode_audio_shaped(fp, ds_org_seq_, ds_org_tran_, lv_org_tran_, js_org_seq_, js_org_tran_, wl_org_seq_, wl_org_tran_, org_trans):
        ...
        return audio, ds_org_seq_, ds_org_tran_, lv_org_tran_, js_org_seq_, js_org_tran_, wl_org_seq_, wl_org_tran_, mask, mask_freq, org_trans

        # return audio, ds_org_seq_, ds_org_tran_, lv_org_tran_, js_org_seq_, js_org_tran_, mask, mask_freq
    ...
    #                                                tf.cast(0, tf.float32), tf.cast(0, tf.float32)),
    #                                drop_remainder=True)
    # dataset = dataset.padded_batch(batch_size=batch_size,
    #                                padded_shapes=([audio_data_maxlen, decode_num_channels], [ds_tran_maxlen], [], [],
    #                                               [js_tran_maxlen], [wl_tran_maxlen], [audio_data_maxlen], [max_length_freq, 80]),
    #                                padding_values=(tf.cast(0, tf.float32), tf.cast(0, tf.int32), tf.cast("", tf.string),
    #                                                tf.cast("", tf.string), tf.cast(0, tf.int32), tf.cast(0, tf.int32),
    #                                                tf.cast(0, tf.float32), tf.cast(0, tf.float32)),
    #                                drop_remainder=True)
    # 这里的dataset要和 _decode_audio_shaped返回结果对应
    dataset = dataset.padded_batch(batch_size=batch_size,
                                   padded_shapes=([audio_data_maxlen, decode_num_channels], [ds_tran_maxlen], [], [],
                                                  [js_tran_maxlen], [], [wl_tran_maxlen], [], [audio_data_maxlen],
                                                  [max_length_freq, 80], []),
                                   padding_values=(tf.cast(0, tf.float32), tf.cast(0, tf.int32), tf.cast("", tf.string),
                                                   tf.cast("", tf.string), 
                                                   tf.cast(0, tf.int32), tf.cast("", tf.string), tf.cast(0, tf.int32),
                                                   tf.cast("", tf.string), tf.cast(0, tf.float32), 
                                                   tf.cast(0, tf.float32), tf.cast("", tf.string)),
                                   drop_remainder=True)
...
# def get_iterator(handle, train_bs, test_bs, train_wav_dir, train_tran_txt,
#                  test_wav_dir=None, test_tran_txt=None):
def get_iterator(handle, train_bs,  train_wav_dir, train_tran_txt):
    ...
    with tf.name_scope("decode"):
        ...
        iterator = tf.data.Iterator.from_string_handle(string_handle=handle, output_types=train_dataset.output_types,
                                                       output_shapes=train_dataset.output_shapes)
    # return iterator, train_iterator, test_iterator
    return iterator, train_iterator
"""
...
if __name__ == "__main__":
    ...
    training_wav_dir = "/dataset/recaptchaV2/train/wav"
    validation_wav_dir = "/data/recaptchaV2/val/wav"
    training_tran_txt = '/dataset/val/tran_d_l_j_w_p.txt'  # 训练集转录文件
    validation_tran_txt = '/dataset/val/tran_d_l_j_w_p.txt'
