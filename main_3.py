...
def train(args, train_dir_,  summary_dir_, model_dir_, config_file, train_tran_txt_,  result_dir_):  # args是参数集
    ...
    with tf.name_scope("Train_opt"):
        ...
        wt = tf.Variable(initial_value=np.array([1, 8, 5, 8, 5, 1]), dtype=tf.float32, name='other_wt')
    ...
    other_var = [x for x in tf.global_variables() if x not in generator_var + jasper_var]
    ...
    with tf.Session(config=config) as sess:
        ...
        # 训练时将下面两行注释，测试时需要取消注释，用于加载训练好的（生成器和其他）模型
        ...
        total_steps = 200000  # 设定的总迭代次数

        while step <= total_steps:
            print("print-step~ {0}/{1}".format(total_steps, step))
            ...
            if step % 100 == 0:
                ...
                while True:  # 对验证集上的所有数据进行测试
                    try:
                        # 这里进行jasper验证，尝试输出jasper的output 并在后面得到文本转录
                        ...
                        jot = [str(i, encoding='utf-8').lower() for i in val_r[8]]  # jasper原始音频转录
                        jat = js_preds(val_r[9])
                        ...
                        if not args.targeted:  # 无目标（原转录vs对抗转录；期望存在相同的转录数为0， 存在不同的是batchsize）
                            ...
                            overlap_same_v += max(lv_same, ds_same, js_same)
                            overlap_diff_v += min(lv_diff, ds_diff, js_diff)
                            ...
                            overlap_same_v += min(lv_same, ds_same, js_same)
                            overlap_diff_v += max(lv_diff, ds_diff, js_diff)
                ...
                print("print~ 目前存在相同和不相同数: {0} vs {1}".format(cur_same, cur_diff))
                print("print~ 目前信噪比均值：{0}----{1}".format(cur_snr, avg_snr))
                print("val_loop: {0}".format(val_loop))

            if step % 100 == 0:
                # 将几种损失值以及扰动结果存入文档，用以绘图观察变化情况
                with open(os.path.join(result_dir_, "loss.txt"), 'a') as f_loss:
                    ...
                    f_loss.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\n".
                                 format(gl, js_al, l2l, pl, np.sum(np.abs(delta)), avg_snr))

                print("print~ 扰动最大、最小、和值：{0}\t{1}\t{2}".format(np.max(delta), np.min(delta), np.sum(delta)))
                print("print~ 信噪比均值：{0}".format(avg_snr))
                # print("print~ lingvo-loss: {0}\tdeepspeech-loss: {1}\tl2loss: {2}".format(lv_al, ds_al, l2l))
                print("print~ lingvo-loss: {0}\tdeepspeech-loss: {1}\tjasper-loss: {2}\tl2loss: {3}\tall_Gloss:{4}".format(lv_al, ds_al, js_al, l2l, all_gl))
            ...
            # 保存模型和summary
            if step % 100 == 0:
                if (not args.targeted and overlap_diff_v == val_num) or (args.targeted and overlap_same_v == val_num):
                    if avg_snr > cur_snr:
                        t_model = time.time() - start - val_time
                        g_saver.save(sess, os.path.join(model_dir_, 'g-model-{0:.5f}'.format(t_model)), step)
                        print("---------save model---------------------------------------------")
                        writer.add_summary(ms, global_step=step)
                        cur_snr = avg_snr
                elif (not args.targeted and overlap_diff_v > cur_diff) or (args.targeted and overlap_same_v > cur_same):
                    t_model = time.time() - start - val_time
                    g_saver.save(sess, os.path.join(model_dir_, 'g-model-{0:.5f}'.format(t_model)), global_step=step)
                    print("---------save model---------------------------------------------")
                    writer.add_summary(ms, global_step=step)
                    ...
                    # if avg_snr > cur_snr:
                    cur_snr = avg_snr
                elif (not args.targeted and overlap_diff_v == cur_diff) or (
                        args.targeted and overlap_same_v == cur_same):
                    if avg_snr >= cur_snr:
                        t_model = time.time() - start - val_time
                        g_saver.save(sess, os.path.join(model_dir_, 'g-model-{0:.5f}'.format(t_model)),
                                     global_step=step)
                        print("---------save model---------------------------------------------")
                        writer.add_summary(ms, global_step=step)
                        cur_same = overlap_same_v
            ...
            # 运行终止条件（上面的条件是严格版，下面的是宽松版）
            # if avg_snr > 20 and ((not args.targeted and overlap_same_v == 0)
            #                      or (args.targeted and overlap_diff_v == 0)):
            if avg_snr > 20 and ((not args.targeted and overlap_diff_v == val_num)
                                 or (args.targeted and overlap_same_v == val_num)):
                print("满足终止条件，停止运行！", file=sys.stderr)
                t_end = time.time() - start - val_time
                g_saver.save(sess, os.path.join(model_dir_, 'g-model-{0:.5f}'.format(t_end)), global_step=step)
                writer.add_summary(ms, global_step=step)
                break
    ...
    with tf.Session(config=config) as sess:
        ...
        while step <= 20000:
            ...
            if step % 100 == 0:
                ...
                _, ms, delta, gl, ds_al, lv_al, js_al, l2l, slen, pl, all_gl = val_r[12:]
                ...
                print("***deepspeech loss:",ds_al, " ***lingvo loss:", lv_al, " ***jasper loss:", js_al)
            ...
            if step % 100 == 0:
                with open(os.path.join(result_dir_, "loss.txt"), 'a') as f_loss:
                    # f_loss.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\n".
                    #              format(gl, ds_al, lv_al, l2l, pl, np.sum(np.abs(delta)), avg_snr))
                    f_loss.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n".
                                 format(gl, ds_al, lv_al, js_al, l2l, pl, np.sum(np.abs(delta)), snrs_t))
...
def mains():
    ...
    parser.set_defaults(
        ...
        training_tran_txt='/media/ps/data/gxy/Adv_audio/data/recaptchaV2/org200/tran_d_l_j_p.txt',  # 训练集转录文件（测试时，只需将验证的路径改为测试集的）
