        ...
        while step <= 60000:
            ...
            if step % 100 == 0:
                ...
                if not args.targeted:  # 无目标（原转录vs对抗转录；期望存在相同的转录数为0， 存在不同的是batchsize）
                    ...
                    overlap_same_v += ds_same     # 如果只用lingvo
                ...
                fetchs = [G_train_op, merged_summary, disturb, G_loss, deepspeech_adv_loss,
                        l2_loss, seq_len, penalty_loss, gloss]
                _, ms, delta, gl, ds_al, l2l, slen, pl, all_gl = \
            ...
            if step % 100 == 0:
                with open(os.path.join(result_dir_, "loss.txt"), 'a') as f_loss:
                    ...
                    f_loss.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\n".
                                 format(gl, ds_al, l2l, pl, np.sum(np.abs(delta)), snrs_t))
