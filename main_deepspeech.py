...
def train(args, train_dir_, validation_dir_, summary_dir_, model_dir_,
    ...
    with tf.name_scope("Train_opt"):
        ...
        # wt = tf.Variable(initial_value=np.array([1, 10, 1, 1]), dtype=tf.float32, name='other_wt')
        ...
        # wt = tf.Variable(initial_value=np.array([1, 4, 1, 1]), dtype=tf.float32, name='other_wt')
        # wt = tf.Variable(initial_value=np.array([1, 2, 1, 1]), dtype=tf.float32, name='other_wt')
        # wt = tf.Variable(initial_value=np.array([1, 12, 1, 1]), dtype=tf.float32, name='other_wt')
        wt = tf.Variable(initial_value=np.array([1, 14, 1, 1]), dtype=tf.float32, name='other_wt')
    ...
    with tf.Session(config=config) as sess:
        ...
        while step <= total_steps:
            ...
            #         lingvo_adv_loss, l2_loss, seq_len, penalty_loss]
            fetchs = [G_train_op, merged_summary, disturb, G_loss, deepspeech_adv_loss, l2_loss, seq_len, penalty_loss]
            # _, ms, delta, gl, ds_al, lv_al, l2l, slen, pl = sess.run(fetchs, feed_dict={handle: training_handle})
            _, ms, delta, gl, ds_al, l2l, slen, pl = sess.run(fetchs, feed_dict={handle: training_handle})
            ...
            if step % 100 == 0:
                ...
                with open(os.path.join(result_dir_, "loss.txt"), 'a') as f_loss:
                    ...
                    f_loss.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\n".
                                 format(gl, ds_al, l2l, pl, np.sum(np.abs(delta)), avg_snr))
                ...
                print("print~ deepspeech-loss: {0}\tl2loss: {1}".format(ds_al, l2l))
                with open(validation_tran_txt_, mode="a") as fp:
                    txt_time = time.time() - start - val_time
                    fp.write("step-{0}-{1:.5f}-{2}-{3}--{4}\n".format(step, txt_time, overlap_same_v, overlap_diff_v, avg_snr))
            ...
            if avg_snr > 20 and ((not args.targeted and overlap_diff_v == val_num) or (args.targeted and overlap_same_v == val_num)):
...
def mains():
    ...
    parser.set_defaults(
        ...
        validation_tran_txt='./data/recaptchaV2/recaptcha5k/tranval_d_l_p.txt',  # 验证集转录文件
        ...
        model_path="./result/recaptcha/07-20_wgan-gptarget/model",
        # other_model_path="./result/recaptcha/03-24-21-56_wgan-gptarget_libri/model_other",
        other_model_path="./result/recaptcha/07-20_wgan-gptarget/model_other",
