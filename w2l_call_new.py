...
def sparse_tensor_to_chars(tensor, idx2char):
  print("tensor.dense_shape[0]:",tensor.dense_shape[0])
...
"""
def call_w2l(config_file, audios_, tgts_, tran_j_, audio_data_maxlen_, tran_save_txt_):
    batch_size = 1
    ...
    with tf.Session(config=config) as sess:
        ...
        w2l_saver.restore(sess, save_path = "/media/ps/data/gxy/Adv_audio/w2lplus_model/model.ckpt-109800")
        ...
        for i in range(loops):
            ...
            for r in range(len(decode)):
                ...
                if tran_j_ is not None:
                    ...
                    print("{0}:\t{1}:\t{2}:\t{3}:\t{4}:\t{5}".format(tgt_np[r], *tran_j_[tgt_np[r]], decode[r]))
...
def w2l_main(config_file, audio_dir_, tran_txt_=None, tran_j_txt_=None):
    ...
    if tran_txt_ is not None:
        ...
        tran_j = {}
        for line in lines:
            name, org_tran, ds_tran, lv_tran, js_tran = line.strip().split(':')
            # tran_j[name] = org_tran
            tran_j[name] = (org_tran, ds_tran, lv_tran, js_tran)
    else:
        tran_j = None
    ...
    call_w2l(config_file, audios, tgts, tran_j, audio_data_maxlen, tran_save_txt_=tran_j_txt_)
...
if __name__ == "__main__":
    ...
    # tran_txt = '/media/ps/data/gxy/Adv_audio/data/recaptchaV2/org200/tran_d_l_j.txt'
    # audio_dir = "/media/ps/data/gxy/Adv_audio/data/recaptchaV2/org200/new_crop"  
    # tran_j_txt = "/media/ps/data/gxy/Adv_audio/data/recaptchaV2/org200/tran_d_l_j_w.txt"
    tran_txt = '/media/ps/data/gxy/Adv_audio/data/digit/tran_d_l_j.txt'
    audio_dir = "/media/ps/data/gxy/Adv_audio/data/digit/uniform"  
    tran_j_txt = "/media/ps/data/gxy/Adv_audio/data/digit/tran_d_l_j_w.txt"
    config_file = "/media/ps/data/gxy/Adv_audio/w2l_test.py"
    w2l_main(config_file, audio_dir, tran_txt_=tran_txt, tran_j_txt_=tran_j_txt)
