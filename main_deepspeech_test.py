...
def train(args, train_dir_, validation_dir_, summary_dir_, model_dir_,
          train_tran_txt_, validation_tran_txt_, result_dir_):  # args是参数集
    ...
    org_trans = next_element[6] # adding
    ...
    with tf.name_scope("Train_opt"):
        ...
        # wt = tf.Variable(initial_value=np.array([1, 10, 1, 4, 1]), dtype=tf.float32, name='other_wt')
        # wt = tf.Variable(initial_value=np.array([1, 1, 1, 1]), dtype=tf.float32, name='other_wt')
        # wt = tf.Variable(initial_value=np.array([1, 20, 1, 1]), dtype=tf.float32, name='other_wt')
        # wt = tf.Variable(initial_value=np.array([1, 15, 1, 1]), dtype=tf.float32, name='other_wt')
        # wt = tf.Variable(initial_value=np.array([1, 11, 1, 1]), dtype=tf.float32, name='other_wt')
        # wt = tf.Variable(initial_value=np.array([1, 9, 1, 1]), dtype=tf.float32, name='other_wt')
        wt = tf.Variable(initial_value=np.array([1, 8, 1, 1]), dtype=tf.float32, name='other_wt')
        # gloss = tf.reduce_sum(wt * tf.stack([G_loss, deepspeech_adv_loss, lingvo_adv_loss, l2_loss, penalty_loss]))
        gloss = tf.reduce_sum(wt * tf.stack([G_loss, deepspeech_adv_loss, l2_loss, penalty_loss]))
    ...
    other_var = [x for x in tf.global_variables() if x not in generator_var + deepspeech_var]
            ...
            if step % 100 == 0:
                ...
                while True:  # 对验证集上的所有数据进行测试
                    try:
                        ...
                        # fetches = [x, ax, ds_org_trans, deepspeech_logits_lm, ds_decodes[0], seq_len, lv_org_trans]
                        # fetches = [x, ax, ds_org_trans, deepspeech_logits_lm, ds_decodes[0], seq_len, lv_org_trans]
                        fetches = [x, ax, ds_org_trans, deepspeech_logits_lm, ds_decodes[0], seq_len, org_trans]
                        ...
                        org =  [str(i, encoding='utf-8').lower() for i in val_r[6]]
                        ...
                        for n in range(args.validation_batch_size):
                            ...
                            if step % 500 == 0:
                                for fn in f_name_l:
                                    audio_name = str(step) + "_" + str(val_loop) + '_' + str(n) + '_' + ''.join(org[n]) + ".wav" # adding
...
def mains():
    ...
    parser.set_defaults(
        ...
        # training_wav_dir="./data/recaptchaV2/recaptcha1w/train",  # 训练集音频文件目录
        # validation_wav_dir="./data/recaptchaV2/recaptcha1w/test",  # 验证集音频文件所在目录
        # training_tran_txt='./data/recaptchaV2/recaptcha1w/trantrain_d_l_p.txt',  # 训练集转录文件（测试时，只需将验证的路径改为测试集的）
        # validation_tran_txt='./data/recaptchaV2/recaptcha1w/trantest_d_l_p.txt',  # 验证集转录文件
        training_wav_dir="./data/recaptchaV2/recaptcha5k/train",  # 训练集音频文件目录
        validation_wav_dir="./data/recaptchaV2/recaptcha5k/test",  # 验证集音频文件所在目录
        training_tran_txt='./data/recaptchaV2/recaptcha5k/trantrain_d_l_p.txt',  # 训练集转录文件（测试时，只需将验证的路径改为测试集的）
        validation_tran_txt='./data/recaptchaV2/recaptcha5k/trantest_d_l_p.txt',  # 验证集转录文件
        ...
        # model_path="./result/recaptcha/07-11_wgan-gptarget/07-12-10-53_wgan-gp/model",
        # model_path="./result/recaptcha/07-15_wgan-gptarget/07-16-13-35_wgan-gp/model",
        # model_path="./result/recaptcha/07-15_wgan-gptarget/07-18-17-33_wgan-gp/model",
        # model_path="./result/recaptcha/07-20_wgan-gptarget/07-20-22-00_wgan-gp/model",
        # model_path="./result/recaptcha/07-20_wgan-gptarget/07-23-15-06_wgan-gp/model",
        # model_path="./result/recaptcha/07-20_wgan-gptarget/07-25-19-56_wgan-gp/model",
        # model_path="./result/recaptcha/12-21-09-35_wgan-gp/model",
        # model_path="/media/ps/data/gxy/Adv_audio/result/recaptcha/12-21-09-38_wgan-gp/model",
        model_path="/media/ps/data/YZN/Adv_audio2/result/recaptcha/07-21_wgan-gptarget/07-23-15-08_wgan-gp/model",
        # other_model_path="./result/recaptcha/03-24-21-56_wgan-gptarget_libri/model_other",
        # other_model_path="./result/recaptcha/07-11_wgan-gptarget/07-12-10-53_wgan-gp/model_other",
        other_model_path="/media/ps/data/YZN/Adv_audio2/result/recaptcha/07-21_wgan-gptarget/07-23-15-08_wgan-gp/model_other",
    )
    ...
    # result_dir = os.path.join("./result/recaptcha/07-11_wgan-gptarget/07-12-10-53_wgan-gp/")
    # result_dir = os.path.join("./result/recaptcha/07-15_wgan-gptarget/07-18-17-33_wgan-gp/")
    # result_dir = os.path.join("./result/recaptcha/07-20_wgan-gptarget/07-20-22-00_wgan-gp/")
    # result_dir = os.path.join("./result/recaptcha/07-20_wgan-gptarget/07-23-15-06_wgan-gp/")
    # result_dir = os.path.join("./result/recaptcha/07-20_wgan-gptarget/07-25-19-56_wgan-gp/")
    # result_dir = os.path.join("./result/recaptcha/07-20_wgan-gptarget/07-30-19-31_wgan-gp/")
    # result_dir = os.path.join("./result/recaptcha/07-20_wgan-gptarget/07-30-19-31_wgan-gp/")
    # result_dir = os.path.join("./result/recaptcha/12-21-09-35_wgan-gp")
    # result_dir = os.path.join("/media/ps/data/gxy/Adv_audio/result/recaptcha/12-21-09-38_wgan-gp/")
    result_dir = os.path.join("/media/ps/data/YZN/Adv_audio2/result/recaptcha/07-21_wgan-gptarget/07-23-15-08_wgan-gp/")
    ...
    validation_tran_txt = os.path.join(result_dir, "test_tran.txt")
    ...
    if not os.path.exists(model_dir):
        os.mkdir(model_dir)
    train(args, train_dir, validation_dir, summary_dir, model_dir, train_tran_txt, validation_tran_txt, result_dir)
