...
def variable_on_worker_level(name, shape, initializer):
    ...
    with tf.device(device):
        # Create or get apropos variable，创建或者获取该name的变量
...
def BiRNN(batch_x, seq_length, dropout, reuse=False, batch_size=None, n_steps=-1, previous_state=None, tflite=False):
    ...
    # Input shape: [batch_size, n_steps, n_input + 2*n_input*n_context]  # shape(bs, 帧数, 19, 26)
    #                             帧数      26          26       9
    if not batch_size:
    ...
    batch_x = tf.transpose(batch_x, [1, 0, 2, 3])  # 交换输入的第0、1维，shape(帧数, bs, 19, 26)
    # Reshape to prepare input for first layer， n_input=26， n_context=9   # shape(bs*帧数, 26+2*26*9)
    batch_x = tf.reshape(batch_x, [-1, Config.n_input + 2 * Config.n_input * Config.n_context])
    layers['input_reshaped'] = batch_x

    # The next three blocks will pass `batch_x` through three hidden layers with
    # clipped RELU activation and dropout.
    ###### 先是经过 3 个隐藏层（全连接，w*x+b）+ clip_relu + dropout

    # 1st layer， n_hidden_1=2048
    b1 = variable_on_worker_level('b1', [Config.n_hidden_1], tf.zeros_initializer())  # 偏置shape(2048,)
    h1 = variable_on_worker_level('h1', [Config.n_input + 2 * Config.n_input * Config.n_context, Config.n_hidden_1],
                                  tf.contrib.layers.xavier_initializer())  # 权重shape(26+2*26*9, 2048)
    layer_1 = tf.minimum(tf.nn.relu(tf.add(tf.matmul(batch_x, h1), b1)), FLAGS.relu_clip)  # shape(bs*帧数, 2048)

    # layer_1中的值有dropout[0]的概率将变为0，其他值变为layer_1/该概率。
    ...
    layer_2 = tf.nn.dropout(layer_2, (1.0 - dropout[1]))  # shape(bs*帧数, 2048)
    ...
    layer_3 = tf.nn.dropout(layer_3, (1.0 - dropout[2]))  # shape(bs*帧数, 2048)
    ...
    ###### 然后是一个lstm层
    # Forward direction cell:  前向的细胞状态
    if not tflite:
    ...
    layer_3 = tf.reshape(layer_3, [n_steps, batch_size, Config.n_hidden_3])  # shape(帧数, bs, 2048)
    if tflite:
    ...
    if not tflite:  # output.shape(帧数, bs, 2048)
    ...
    output = tf.reshape(output, [-1, Config.n_cell_dim])  # shape(帧数*bs, 2048)
    layers['rnn_output'] = output
    layers['rnn_output_state'] = output_state

    # Now we feed `output` to the fifth hidden layer with clipped RELU activation and dropout
    ###### 然后是一个隐藏层（全连接+clipped_relu+dropout），  n_hidden_5=2048， n_cell_dim=2048
    ...
    layer_5 = tf.nn.dropout(layer_5, (1.0 - dropout[5]))  # shape(帧数*bs, 2048)
    layers['layer_5'] = layer_5

    # Now we apply the weight matrix `h6` and bias `b6` to the output of `layer_5`
    # creating `n_classes` dimensional vectors, the logits.
    ###### 最后是一个全连接层， n_hidden_6=29，表示最后一层的输出类别有29类，分别是（ abcdefghijklmnopqrstuvwxyz'-）
    ...
    layer_6 = tf.add(tf.matmul(layer_5, h6), b6)  # shape(帧数*bs, 29)
    layers['layer_6'] = layer_6

    # Finally we reshape layer_6 from a tensor of shape [n_steps*batch_size, n_hidden_6]
    # to the slightly more useful shape [n_steps, batch_size, n_hidden_6].
    # Note, that this differs from the input in that it is time-major.
    layer_6 = tf.reshape(layer_6, [n_steps, batch_size, Config.n_hidden_6], name="raw_logits")  # shape(帧数, bs, 29)
