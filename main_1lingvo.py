...
def train(args, train_dir_, summary_dir_, model_dir_, train_tran_txt_, result_dir_):  # args是参数集
    ...
    with tf.name_scope("Train_opt"):
        ...
        # wt = tf.Variable(initial_value=np.array([1, 0.1, 2, 1]), dtype=tf.float32, name='other_wt')
        wt = tf.Variable(initial_value=np.array([1, 1, 1, 1]), dtype=tf.float32, name='other_wt')
        ...
        gloss = tf.reduce_sum(wt * tf.stack([G_loss, lingvo_adv_loss, l2_loss, penalty_loss]))
    ...
    with tf.Session(config=config) as sess:
        ...
        while step <= 60000:
            ...
            if step % 100 == 0:
                ...
                fetches = [x, ax, org_trans, ds_org_trans, lv_org_trans, lingvo_decoded,
                           G_train_op, merged_summary, disturb, G_loss,
                           lingvo_adv_loss, l2_loss, penalty_loss, gloss]
                val_r = sess.run(fetches, feed_dict={handle: training_handle})
                _, ms, delta, gl, lv_al, l2l, pl, all_gl = val_r[6:]

                org = [str(i, encoding='utf-8').lower() for i in val_r[2]]

                # 每**次迭代，查看转录结果
                lot = [str(i, encoding='utf-8').lower() for i in val_r[4]]  # lingvo原音频转录
                lat = [str(i, encoding='utf-8').lower() for i in val_r[5]['topk_decoded'][:, 0]]  # lingvo对抗转录
                ...
                fetchs = [G_train_op, merged_summary, disturb, G_loss,
                          lingvo_adv_loss, l2_loss, penalty_loss, gloss]
                _, ms, delta, gl, lv_al, l2l, pl, all_gl = \
            ...
            if step % 100 == 0:
                with open(os.path.join(result_dir_, "loss.txt"), 'a') as f_loss:
                    ...
                    f_loss.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\n".
                                 format(gl, lv_al, l2l, pl, np.sum(np.abs(delta)), snrs_t))
