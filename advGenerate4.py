...
def train(args, train_dir_, validation_dir_, summary_dir_, model_dir_, config_file, config_file_wl,
    ...
    wl_org_tran_seq = next_element[6]
    wl_org_trans = next_element[7]
    ...
    wl_tran_maxlen = tf.shape(wl_org_tran_seq)[1]
    ...
    with tf.variable_scope("WaveLetter"):
        w2l_model = create_model(args_wl, base_config_wl, config_module_wl, base_model_wl, hvd=None,
                                     checkpoint=None)
    ...
    with tf.Session(config=config) as sess:
        ...
        while True:
            try:
                fetches = [x, ax, org_trans, ds_org_trans, deepspeech_logits_lm, ds_decodes[0], seq_len, lv_org_trans,
                           lingvo_decoded, js_org_trans, model_outputs[0], wl_org_trans, model_outputs_wl[0]]
                ...
                wot = [str(i, encoding='utf-8').lower() for i in val_r[11]]  # w2l原始音频转录
                wat = js_preds(val_r[12])
                ...
                wl_same, wl_diff = str_list_cmp(org, wat)
                ...
                for bs in range(args.validation_batch_size):
                    ...
                    fp.write("step-{0}-{1}-{2}-org_wl: {3}\n".format(step, val_loop, bs, wot[bs]))
                    fp.write("step-{0}-{1}-{2}-adv_wl: {3}\n\n".format(step, val_loop, bs, wat[bs]))
        ...
        print("****** deepspeech成功：", ds_diff_all)
        print("****** lingvo成功：", lv_diff_all)
        print("****** jasper成功：", js_diff_all)
        print("****** w2l成功：", wl_diff_all)
...
def mains():
    ...
    parser.set_defaults(
        ...
        validation_tran_txt='/media/ps/data/gxy/Adv_audio/data/digit/tran_d_l_j_w_p.txt',  # 验证集转录文件
        # training_tran_txt='/media/ps/data/gxy/Adv_audio/data/recaptchaV2/org200/tran_d_l_j_w_p.txt',
        # validation_tran_txt='/media/ps/data/gxy/Adv_audio/data/recaptchaV2/org200/tran_d_l_j_w_p.txt',  # 验证集转录文件
        ...
        # model_path="./result/recaptcha/11-15-14-36_wgan-gp/model",
        # model_path="./result/recaptcha/11-18-10-28_wgan-gp/model",
        # model_path="./result/recaptcha/11-24-09-08_wgan-gp/model",
        model_path="./result/recaptcha/11-23-09-09_wgan-gp/model",
        # model_path="./result/recaptcha/11-18-10-54_wgan-gp/model",
    ...
    # result_dir = os.path.join("./result/recaptcha/11-18-10-28_wgan-gp")
    # result_dir = os.path.join("./result/recaptcha/11-24-09-08_wgan-gp")
    result_dir = os.path.join("./result/recaptcha/11-23-09-09_wgan-gp")
    # result_dir = os.path.join("./result/recaptcha/11-18-10-54_wgan-gp")
    ...
    train(args, train_dir, validation_dir, summary_dir, model_dir, config_file_wl, config_file, train_tran_txt,
          validation_tran_txt, result_dir)
