...
class _Cluster(object):
  ...
  def Params(cls):
    ...
    p.Define(
        'mode', 'async_new', 'A string noting the overall training method. '
  ...
  def __init__(self, params):
    ...
    if p.mode == 'async_new' and p.job == 'controller':
      # There is only 1 controller.
      assert p.controller.replicas == 1
      assert p.task == 0
    elif p.mode == 'async_new' and p.job == 'trainer':
      ...
      if p.ps.replicas == 0:
        ...
        assert p.worker.replicas == 1
    elif p.mode == 'async_new' and p.job == 'evaler':
      assert 0 <= p.task and p.task < p.evaler.replicas
    elif p.mode == 'async_new' and p.job == 'decoder':
  ...
  def async_new(self):
    ...
    return self.params.mode == 'async_new'
  ...
  def available_devices(self):
    ...
    if self.job == 'trainer' and self.async_new:
