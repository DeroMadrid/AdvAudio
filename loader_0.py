...
def get_audio_maxlen(fps):
    ...
    for fp in fps:
        print(fp)
        wav = decode_audio(fp, fs=16000, num_channels=1, normalize=False, fast_wav=True)[0]
        max_l = max(max_l, wav.shape[0])
    return max_l


ff = "./data/recaptchaV2/recaptcha1w/train"
fps = glob(os.path.join(ff, "*"))
print(get_audio_maxlen(fps))
...
def get_fps_tran(wav_dir, tran_txt):
    ...
    # return fps, ds_org_seqs, ds_org_trans, lv_org_trans, ds_org_tran_maxlen
    return fps, ds_org_seqs, ds_org_trans, lv_org_trans, ds_org_tran_maxlen, org_trans
...
def decode_extract_and_batch(wav_dir, tran_txt, shuffle=False, repeat=False, batch_size=1, decode_fs=16000,
    ...
    :return: 一个dataset
    """
    # fps, ds_org_seqs, ds_org_trans, lv_org_trans, ds_tran_maxlen = get_fps_tran(wav_dir, tran_txt)
    fps, ds_org_seqs, ds_org_trans, lv_org_trans, ds_tran_maxlen, org_trans = get_fps_tran(wav_dir, tran_txt)
    ...
    # dataset = tf.data.Dataset.from_tensor_slices((fps, ds_org_seqs, ds_org_trans, lv_org_trans))
    dataset = tf.data.Dataset.from_tensor_slices((fps, ds_org_seqs, ds_org_trans, lv_org_trans, org_trans))
    ...
    # def _decode_audio_shaped(fp, ds_org_seq_, ds_org_tran_, lv_org_tran_, ):
    def _decode_audio_shaped(fp, ds_org_seq_, ds_org_tran_, lv_org_tran_, org_tran_ ):
        ...
        return audio, ds_org_seq_, ds_org_tran_, lv_org_tran_, mask, mask_freq, org_tran_
    ...
    # 制作batch。drop_remainde为True表示将分批后多余的部分丢弃，否则将剩余的部分以小于batchsize的大小组一个batch
    # dataset = dataset.batch(batch_size, drop_remainder=True)
    dataset = dataset.padded_batch(batch_size=batch_size,
                                   padded_shapes=([audio_data_maxlen, decode_num_channels], [ds_tran_maxlen], [], [],
                                                  [audio_data_maxlen], [max_length_freq, 80], []),
                                   padding_values=(tf.cast(0, tf.float32), tf.cast(0, tf.int32), tf.cast("", tf.string),
                                                   tf.cast("", tf.string), tf.cast(0, tf.float32), tf.cast(0, tf.float32),
                                                   tf.cast("", tf.string)),
                                   drop_remainder=True)
...
#     tf_masks = tf.expand_dims(next_element[4], -1)
