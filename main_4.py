...
def wl_var(global_variables):
    return [x for x in global_variables if (x.name.startswith('W'))]


def train(args, train_dir_,  summary_dir_, model_dir_, config_file, config_file_wl, train_tran_txt_,  result_dir_):  # args是参数集
    ...
    wl_org_tran_seq = next_element[6]  # wav2letter+原转录的数字序列形式， shape(batch_size, org_tran_maxlen)   # edit by xcx 21-11-10
    wl_org_trans = next_element[7]  # wav2letter+原转录    # edit by xcx 21-11-10
    tf_masks = next_element[8]
    tf_masks_freq = next_element[9]
    org_trans = next_element[10]
    ds_tran_maxlen = tf.shape(ds_org_tran_seq)[1]  # deepspeech原转录的最大长度
    js_tran_maxlen = tf.shape(js_org_tran_seq)[1]  # jasper原转录的最大长度
    wl_tran_maxlen = tf.shape(wl_org_tran_seq)[1]  # w2l原转录的最大长度 # edit by xcx 21-11-10
    ...
    # 这里补充jasper损失计算的过程
    js_src_length = decoder_output.get("src_length", None)
    # 目标转录
    if not args.targeted:
        js_org_tran_seq = tf.clip_by_value(js_org_tran_seq, 0, len(toks()) - 2)
        js_org_tran_len = tf.fill(dims=(args.train_batch_size,), value=tf.cast(js_tran_maxlen, tf.int32))
        js_org_tran_sparse = ctc_label_dense_to_sparse(labels=js_org_tran_seq, label_lengths=js_org_tran_len)
        js_total_loss = tf.nn.ctc_loss(
            labels=tf.cast(js_org_tran_sparse, tf.int32),
            inputs=tf.cast(model_logits, tf.float32),
            sequence_length=js_src_length,
            ignore_longer_outputs_than_inputs=True,
        )
        js_adv_loss = -tf.reduce_mean(js_total_loss)
    else:
        tgt_tran = args.adv_target  # 自定义的目标转录
        # tgt_tran_seq_list = [[idx2char().index(j) for j in tgt_tran] for _ in range(args.train_batch_size)]  # 将目标转录短语转为数字序列形式
        tgt_tran_seq_list = [[char2idx[j] for j in tgt_tran] for _ in range(args.train_batch_size)]
        tgt_tran_seq = tf.constant(np.array([list(t) + [0] * (len(tgt_tran) - len(t)) for t in tgt_tran_seq_list]),
                                   dtype=tf.int32)  # 目标转录
        tgt_tran_len_tf = tf.constant(np.array([len(t) for t in tgt_tran_seq_list]), dtype=tf.int32)
        tgt_tran_sparse = ctc_label_dense_to_sparse(labels=tgt_tran_seq, label_lengths=tgt_tran_len_tf)  # 转化为稀疏Tensor
        ...
        js_total_loss = tf.nn.ctc_loss(
            labels=tf.cast(tgt_tran_sparse, tf.int32),
            inputs=tf.cast(model_logits, tf.float32),
            sequence_length=js_src_length,
            ignore_longer_outputs_than_inputs=True,
        )
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~w2l_loss~~~~~~~~~~~~~~~~~edit by xcx 21-11-1~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # input_tf = tf.clip_by_value(ax[:, :, 0] * 32768 + random_noise, -2 ** 15, 2 ** 15 - 1)
    # 不需要进行归一化

    # 创建模型得到参数
    arg_wl = ['--config_file=' + config_file_wl, '--mode=infer']
    args_wl, base_config_wl, base_model_wl, config_module_wl = get_base_config(arg_wl)

    with tf.variable_scope("WaveLetter"):
        w2l_model = create_model(args_wl, base_config_wl, config_module_wl, base_model_wl, hvd=None, checkpoint=None)

    # 此处得到features，参考deepspeech对tensor类型音频进行logfbank的过程 *可以另开一个file写一个关于特征提取的函数
    ...
    # 这里直接用jasper的
    input_tensors_wl = {}
    input_tensors_wl["source_tensors"] = [jasper_features, jasper_len]
    ...
    for gpu_cnt, gpu_id in enumerate(w2l_model._gpu_ids):
        with tf.device("/gpu:{}".format(gpu_id)), tf.variable_scope(
                "WaveLetter/ForwardPass",
                reuse=tf.AUTO_REUSE,
        ):
            # with tf.variable_scope("ForwardPass"):
            encoder_input_wl = {"source_tensors": input_tensors["source_tensors"]}
            encoder_output_wl = w2l_model.encoder.encode(input_dict=encoder_input_wl)

            decoder_input_wl = {"encoder_output": encoder_output_wl}
            decoder_output_wl = w2l_model.decoder.decode(input_dict=decoder_input_wl)
            model_outputs_wl = decoder_output_wl.get("outputs", None)
            model_logits_wl = decoder_output_wl.get("logits", None)

    # w2l损失计算的过程
    wl_src_length = decoder_output_wl.get("src_length", None)
    # 目标转录
    if not args.targeted:
        wl_org_tran_seq = tf.clip_by_value(wl_org_tran_seq, 0, len(toks()) - 2)
        wl_org_tran_len = tf.fill(dims=(args.train_batch_size,), value=tf.cast(wl_tran_maxlen, tf.int32))
        wl_org_tran_sparse = ctc_label_dense_to_sparse(labels=wl_org_tran_seq, label_lengths=wl_org_tran_len)
        wl_total_loss = tf.nn.ctc_loss(
            labels=tf.cast(wl_org_tran_sparse, tf.int32),
            inputs=tf.cast(model_logits_wl, tf.float32),
            sequence_length=wl_src_length,
            ignore_longer_outputs_than_inputs=True,
        )
        wl_adv_loss = -tf.reduce_mean(wl_total_loss)
    else:
        # tgt_tran_wl = args.adv_target  # 自定义的目标转录
        # tgt_tran_seq_list_wl = [[char2idx[j] for j in tgt_tran_wl] for _ in range(args.train_batch_size)]
        # tgt_tran_seq_wl = tf.constant(np.array([list(t) + [0] * (len(tgt_tran_wl) - len(t)) for t in tgt_tran_seq_list_wl]),
        #                            dtype=tf.int32)  # 目标转录
        # tgt_tran_len_tf_wl = tf.constant(np.array([len(t) for t in tgt_tran_seq_list_wl]), dtype=tf.int32)
        # tgt_tran_sparse_wl = ctc_label_dense_to_sparse(labels=tgt_tran_seq_wl, label_lengths=tgt_tran_len_tf_wl)  # 转化为稀疏Tensor

        # wl_total_loss = tf.nn.ctc_loss(
        #     labels=tf.cast(tgt_tran_sparse_wl, tf.int32),
        #     inputs=tf.cast(model_logits_wl, tf.float32),
        #     sequence_length=wl_src_length,
        #     ignore_longer_outputs_than_inputs=True,
        # )
    ...
    tf.summary.scalar(name='jasper_adv_loss', tensor=js_adv_loss)
    tf.summary.scalar(name='wl_adv_loss', tensor=wl_adv_loss)
    ...
    with tf.name_scope("Train_opt"):
        ...
        # wt = tf.Variable(initial_value=np.array([1, 1, 20, 1, 10, 1]), dtype=tf.float32, name='other_wt')
        # gloss = tf.reduce_sum(wt * tf.stack([G_loss, deepspeech_adv_loss, lingvo_adv_loss, js_adv_loss,
        #                                      l2_loss, penalty_loss]))
        # wt = tf.Variable(initial_value=np.array([1, 1, 20, 1, 1, 8, 1]), dtype=tf.float32, name='other_wt')
        wt = tf.Variable(initial_value=np.array([1, 8, 5, 8, 8, 6, 1]), dtype=tf.float32, name='other_wt')
        gloss = tf.reduce_sum(wt * tf.stack([G_loss, deepspeech_adv_loss, lingvo_adv_loss, js_adv_loss, wl_adv_loss,
                                             l2_loss, penalty_loss]))
    ...
    jasper_var = js_var(tf.global_variables())
    if jasper_var:
        jasper_saver = tf.train.Saver(var_list=jasper_var)

    w2l_var = wl_var(tf.global_variables())
    if w2l_var:
        w2l_saver = tf.train.Saver(var_list=w2l_var)

    other_var = [x for x in tf.global_variables() if x not in generator_var + jasper_var + w2l_var]
    ...
    with tf.Session(config=config) as sess:
        ...
        # 可以考虑在这里补充jasper和wav2letter+的模型
        jasper_saver.restore(sess, save_path="./jasper_model/model.ckpt-438235")

        w2l_saver.restore(sess, save_path="./w2lplus_model_new/model.ckpt-109800")
        ...
        while step <= 20000:
            ...
            if step % 100 == 0:
                ...
                #            lingvo_decoded, js_org_trans, model_outputs[0], model_logits,
                #            G_train_op, merged_summary, disturb, G_loss, deepspeech_adv_loss,
                #            lingvo_adv_loss, js_adv_loss, l2_loss, seq_len, penalty_loss, gloss]
                # val_r = sess.run(fetches, feed_dict={handle: training_handle})
                # _, ms, delta, gl, ds_al, lv_al, js_al, l2l, slen, pl, all_gl = val_r[11:]
                fetches = [x, ax, org_trans, ds_org_trans, deepspeech_logits_lm, ds_decodes[0], seq_len, lv_org_trans,
                           lingvo_decoded, js_org_trans,  model_outputs[0], model_logits, wl_org_trans, model_outputs_wl[0], model_logits_wl,
                           G_train_op, merged_summary, disturb, G_loss, deepspeech_adv_loss,
                           lingvo_adv_loss, js_adv_loss, wl_adv_loss, l2_loss, seq_len, penalty_loss, gloss]
                val_r = sess.run(fetches, feed_dict={handle: training_handle})
                _, ms, delta, gl, ds_al, lv_al, js_al, wl_al, l2l, slen, pl, all_gl = val_r[15:]
                ...
                wot = [str(i, encoding='utf-8').lower() for i in val_r[12]]  # w2l原始音频转录
                wat = js_preds(val_r[13])
                ...
                if not args.targeted:  # 无目标（原转录vs对抗转录；期望存在相同的转录数为0， 存在不同的是batchsize）
                    ...
                    wl_same, wl_diff = str_list_cmp(wot, wat)
                    overlap_same_v += max(lv_same, ds_same, js_same, wl_same)
                    overlap_diff_v += min(lv_diff, ds_diff, js_diff, wl_diff)
                    ...
                    js_same, js_diff = str_list_cmp(jat, target_trans)
                    wl_same, wl_diff = str_list_cmp(wot, target_trans)
                    overlap_same_v += min(lv_same, ds_same, js_same, wl_same)
                    overlap_diff_v += max(lv_diff, ds_diff, js_diff, wl_diff)
                ...
                for bs in range(args.train_batch_size):
                    ...
                    fp.write("step-{0}-{1}-org_js: {2}\n".format(step, bs, jot[bs]))
                    fp.write("step-{0}-{1}-adv_js {2}\n".format(step, bs, jat[bs]))
                    fp.write("step-{0}-{1}-org_wl: {2}\n".format(step, bs, wot[bs]))
                    fp.write("step-{0}-{1}-adv_wl {2}\n\n".format(step, bs, wat[bs]))
                fp.close()
                ...
                print("***deepspeech loss:",ds_al, " ***lingvo loss:", lv_al, " ***jasper loss:", js_al, " ***wave2letter loss:", wl_al)
                ...
                #           lingvo_adv_loss, js_adv_loss, l2_loss, seq_len, penalty_loss, gloss]
                # _, ms, delta, gl, ds_al, lv_al, js_al, l2l, slen, pl, all_gl = \
                #     sess.run(fetchs, feed_dict={handle: training_handle})
                fetchs = [G_train_op, merged_summary, disturb, G_loss, deepspeech_adv_loss,
                          lingvo_adv_loss, js_adv_loss, wl_adv_loss, l2_loss, seq_len, penalty_loss, gloss]
                _, ms, delta, gl, ds_al, lv_al, js_al, wl_al, l2l, slen, pl, all_gl = \
                    sess.run(fetchs, feed_dict={handle: training_handle})
            ...
            if step % 100 == 0:
                with open(os.path.join(result_dir_, "loss.txt"), 'a') as f_loss:
                    ...
                    # 这个{}数量应该和format里面内容的个数一致
                    f_loss.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\n".
                                 format(gl, ds_al, lv_al, js_al, wl_al, l2l, pl, np.sum(np.abs(delta)), snrs_t))
...
def mains():
    ...
    parser.set_defaults(
        ...
        # training_wav_dir="/dataset/val/wav",  # 训练集音频文件目录
        training_wav_dir="/media/ps/data/gxy/Adv_audio/data/recaptchaV2/org200/new_crop",  # 训练集音频文件目录
        # training_wav_dir="/media/ps/data/gxy/Adv_audio/data/digit/uniform",  # 训练集音频文件目录
        ...
        # training_tran_txt='/dataset/val/tran_d_l_j_w_p.txt',  # 训练集转录文件（测试时，只需将验证的路径改为测试集的）
        training_tran_txt='/media/ps/data/gxy/Adv_audio/data/recaptchaV2/org200/tran_d_l_j_w_p.txt',
        # training_tran_txt='/media/ps/data/gxy/Adv_audio/data/digit/tran_d_l_j_w_p.txt',
    ...
    result_dir = os.path.join("./result/recaptcha", cur_time)
    ...
    config_file_wl = "/media/ps/data/gxy/Adv_audio/w2l_test.py"
    ...
    #       result_dir)
    train(args, train_dir, summary_dir, model_dir, config_file, config_file_wl, train_tran_txt, result_dir)
