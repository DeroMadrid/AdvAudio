...
def get_fps_tran(wav_dir, tran_txt):
    ...
    """
    fps = glob.glob(os.path.join(wav_dir, '*'))     # 音频文件名组成的列表

    # 统一音频数组的长度，并将其重新保存（注意，音频文件要求单声道、int16、16000Hz、wav文件）
    audios = [read(filename=fp)[1] for fp in fps]
    audio_data_maxlen = max(map(len, audios))
    audios = list(map(lambda k: np.pad(k, (0, audio_data_maxlen-len(k)), 'constant'), audios))
    for fp, audio in zip(fps, audios):
        write(filename=fp, rate=16000, data=audio)
    ...
    return fps, ds_org_seqs, ds_org_trans, lv_org_trans, audio_data_maxlen, ds_org_tran_maxlen
...
def decode_extract_and_batch(wav_dir, tran_txt, shuffle=False, repeat=False, batch_size=1, decode_fs=16000,
    ...
    :return: 一个dataset
    """
    fps, ds_org_seqs, ds_org_trans, lv_org_trans, audio_data_maxlen, ds_tran_maxlen = get_fps_tran(wav_dir, tran_txt)
    ...
    def _decode_audio_shaped(fp, ds_org_seq_, ds_org_tran_, lv_org_tran_):
        ...
        audio = tf.py_func(func=_decode_audio_closure, inp=[fp], Tout=tf.float32, stateful=False, name="decode_audio")
        audio.set_shape([None, decode_num_channels])
        return audio, ds_org_seq_, ds_org_tran_, lv_org_tran_
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        while i < 3:
            print("训练集测试：")
            a, b, c, d = sess.run([x, ds_org_seqs, ds_org_trans, lv_org_trans],
